...
**Chart / diagram dry-render.** `validate_input` and `preview_presentation_plan` also drive svggen's layout/labeling pass for every `chart_value` / `diagram_value` content item **and every diagram surface embedded in a slide's `shape_grid` or in a named pattern's expanded grid (e.g. `chart-insights-split`, paths rooted at `/slides/{i}/pattern`)** — cell `diagram`s, composite cell `sub_diagram`s, and diagrams inside recursively nested sub-grids — merging the resulting `chart.*` / `diagram.*` findings (e.g. `chart.tick_thinned`, `chart.label_clipped`, `chart.legend_overflow_dropped`, `chart.plot_area_collapsed`, `diagram.org_chart_depth_pruned`, `diagram.items_dropped`) into the fit-finding stream. Dry renders use the resolved template placeholder or post-fit grid-cell frame, matching generation; an unresolvable frame falls back to svggen's default. `diagram.items_dropped` refuses the slide because authored content would disappear. Every emitted path is a JSON Pointer into the authored input: content findings use `/slides/{i}/content/{j}/chart_value` or `/diagram_value`; shape-grid findings use the same slidepath convention (e.g. `/slides/0/shape_grid/rows/1/cells/2/diagram`). When svggen reports a renderer-derived field or a generated pattern cell, the path falls back to the nearest authored ancestor, such as `/slides/0/pattern`. Non-chart diagram findings use `diagram.*` codes, even if svggen's shared labeling engine internally names them `chart.*`. Agents see render-time chart issues at validate / preview time without paying for full generation. The strict-fit severity ladder applies identically to the generate path. The svggen top-level helper is `svggen.DryRender(req) ([]Finding, error)`; the corresponding MCP entry point is `render_diagram` with `dry_run: true`.
...
Preflight prediction that the renderer's contrast auto-fix pass would replace a text color to meet WCAG AA for its size: 4.5:1 for normal text, 3:1 only for text ≥18pt or ≥14pt bold. Emitted by `validate` and `preview_presentation_plan` (and downstream tools like `repair_slide` / `score_candidates`) using theme colors and JSON content — no PPTX rendering. Shape-grid prediction compiles the text-bearing cells with the normal shape XML writer and runs the **same contrast pass** as generation, including the smallest run in a cell, the white-text-safe allowlist, and sibling-cell color grouping. One shared group decision produces one finding, without an unsafe single-cell repair suggestion.
...
The detector also walks **placeholder text on a background the slide sets itself** (`source: "slide_background"`). Nothing in the JSON names that text's colour — it is the template's, resolved from the layout or master's `txStyles`, including the layout color-map override and OOXML luminance/tint/shade/alpha modifiers. Only the background is the author's, so the shape-grid walk never saw this case. Preflight also applies the same section-number injection rule as generation: a numbered section slide checks its decorative number even when the JSON omits that content item. Unmapped template prompt text is not predicted because generation clears it. The background follows the renderer's own rule (`background.color`, or the scrim colour when an overlay is opaque enough to decide what the text sits on); `contrast_check: false` suppresses the finding.
...
**Provenance affects the replacement** (go-slide-creator-s7wmh). For placeholder text on a background the AUTHOR introduced — `background.color`, or an opaque scrim — the template's text colour was never chosen against it, so the fix snaps to the palette. On the template's own background the hue is intentional and can be preserved by a `lerp`. Shape-grid text uses the same run-size threshold whether its cell has a solid fill or inherits the slide background; neutral white/black text may still flip to a readable theme color. Before this rule a navy title on an author-set black lerped to a mid-grey at exactly the WCAG floor and rendered barely visible; it now snaps to the template's `lt1` (ratio 1.5 → 21).
