...
The remaining designer template `business-template` still fails `template-check` while its repair lands under the [`go-slide-creator-vqad`](https://github.com/) umbrella. Its SHA-256 fingerprint is listed in `internal/template/testdata/conformance_allowlist.json` and `canonical_placeholders_allowlist.json`; the allow-list entry auto-expires when the fingerprint changes after repair.
