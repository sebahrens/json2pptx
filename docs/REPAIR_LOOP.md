...
| `split_bullets` | `max_items` (positive integer per-column budget) | Agent-initiated preservation of dense native plain bullet columns | **Yes** — coordinates equally sized nonempty columns, preserves exact strings and nested groups, repeats other content, keeps notes/source on page one. Refuses compound lists, blank bullets, orphan children, path targeting, oversized nested groups and decks with numeric internal slide links. Render every resulting page; item count is not proof of fit. |
