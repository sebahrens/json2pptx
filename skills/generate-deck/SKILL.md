...
|---|---|---|---|
| Discover | `list_deck_archetypes`, `list_slide_kinds` | `semantic schema` | Enumerate `meta.archetype` / `slides[].kind`. `list_slide_kinds` defaults to a compact catalog with each kind's summary, required + typical fields, `required_aliases` (e.g. `kpi_snapshot` accepts `metrics` for `kpis`), and one copy-ready `example`. Request `kinds:["kpi_snapshot"], fields:["item_schema","compositions"]` for a selected kind's closed schema and supported pattern/layout overrides; an unavailable override reports `SEMANTIC_PATTERN_NOT_AVAILABLE`. `semantic schema` prints the full DeckSpec JSON Schema (draft 2020-12); **`validate_deck_spec`** carries it as its MCP `spec` input schema. `render_deck_spec` / `compile_deck_spec` / `explain_deck_spec` declare the outline and point at this discovery tool. All four accept `spec` as an object or YAML/JSON document string. Unknown payload fields still report `SEMANTIC_UNKNOWN_FIELD`. |

MCP calls reject supplied top-level argument values that do not match their declared type or enum with `INVALID_PARAMETER` and the offending path. Use a JSON number for `slide_index`/`max_slides`, a boolean for `fit_report`, and `off`, `warn`, or `strict` for `strict_fit`; string lookalikes are not coerced.
Schema-aware clients can also validate repair `fixes`, DeckSpec `patch` entries, thumbnail `slide_indices`, and visual `content_hints` before a call. For `validate_deck_spec`, `render_deck_spec`, and `explain_deck_spec`, provide exactly one of `spec` or `deck_id`; `patch` requires `deck_id`.
...
| `kpi_snapshot` | `kpis` (2–6 → cards) | `title`, `takeaway` | each KPI: `{value, label, delta?}` — `value` ≤12 **characters**, `label` ≤40, `delta` ≤12 (e.g. `"+5%"`, renders a small annotation; aliases `sub`/`trend`/`change`). Budgets count characters, so `"EUR 48.25m"` (10) fits; a value past the hard budget degrades the whole slide to a bullet list and says so as `SEMANTIC_PATTERN_DEGRADED`. An in-budget value that cannot fit on one line emits `BODY_TOO_LONG` |
...
| `matrix_2x2` | `quadrants` (exactly 4, clockwise from top left; aliases `cells`, `boxes`, or the named positions `top_left`/`top_right`/`bottom_left`/`bottom_right`), `x_axis`, `y_axis` | `title`, `x_low`, `x_high`, `y_low`, `y_high`, `takeaway` | each quadrant: `"Header"`, `"Header: body"`, or `{header, body?}` — header ≤80 chars, body ≤200. Horizontal axis ≤16 and vertical axis ≤60 (aliases `x_axis_label`, `y_axis_label`); axis ends ≤11. Short of four headed quadrants and two named axes, or past any budget, it degrades to a bullet list naming each quadrant's position and both axes with their ends |
...
| `decision` | `title` | `recommendation`, `options[]` (aliases `choices`, `alternatives`), `takeaway` | each option: `"Label"`, `"Label | detail"`, or `{label, detail?, recommended?}`. **Mark exactly one labeled option `recommended: true`**; it receives an accent-filled Recommended badge. **3–6 options render as numbered boxes** (label ≤60 chars, detail ≤180); **exactly 2, each WITH a detail**, render as two cards side by side (label ≤80, detail ≤300). The `recommendation` is the callout band beneath them — the ask, where the room can see it. One option, seven options, or a two-option pair missing a detail falls back to the recommendation as a lead-in over option bullets |
...
`render_deck_spec` and `compile_deck_spec` parse failures, and render requests for unknown named templates, return the shared `findings[]` envelope with a code. Unknown kinds and templates also include `available[]` and a discovery call (`list_slide_kinds` or `list_templates`); a close template typo has `did_you_mean`. A spec that cannot be parsed receives no `deck_id` from render or validate — correct the source and send it again. Other render/compile failures retain their compact response with `error` and `diagnostics[]`.
...
`compositions` list when requested with `fields:["compositions"]`; everything in that list is an override the kind will honour, and anything
...
- **Tool profile:** by default (`--tools core`) `tools/list` advertises only the 24 core tools (`analyze_deck_rhythm`, `describe_finding`, `examine_template`, `expand_pattern`, `generate_presentation`, `get_capabilities`, `get_input_schema`, `get_started`, `inspect_slide_images`, `list_patterns`, `list_slide_kinds`, `list_templates`, `plan_deck`, `preview_presentation_plan`, `recommend_visual`, `render_deck_spec`, `render_deck_thumbnails`, `render_slide_image`, `repair_slide`, `score_deck`, `show_pattern`, `submit_visual_review`, `validate_deck_spec`, `validate_input`) without `outputSchema` (responses still carry `structuredContent`), keeping the listing under 102KB (the closed DeckSpec schema is embedded once, on `validate_deck_spec`; call `list_slide_kinds` for compact examples, then filter by `kinds` and request `fields:["item_schema"]` only for selected kinds). Start the server with `--tools all` (or env `JSON2PPTX_MCP_TOOLS=all`) to list the full catalogue — facades (`make_deck`, `auto_repair`), `read_presentation`, preview/wireframe, template settings, `score_candidates`, `apply_deck_patch`, etc. Non-core tools remain callable by name in core mode; `get_capabilities().mcp_tools_available[].in_core_profile` tells you which tools the core profile lists.
- **Schema size:** the closed DeckSpec schema on `validate_deck_spec` shares repeated list-entry object definitions through `$defs`, keeping the core `tools/list` around 94KB without removing schema validation. This changes only how the schema is encoded, not which fields compile.
...
|---|---|---|
| `recommend_visual` | PLAN | First call per slide intent — ranks candidates across layouts, patterns, charts, diagrams, and raw shape_grid. Start here when unsure which visual approach fits. Structured `content_hints.has_chart` prefers a matching chart without inventing one; `data_points` prevents overloaded pie/donut or multi-value gauge recommendations, and `audience` adjusts relevant pattern scores without treating audience words as slide content. Pass the optional `template` (a template name) to make it **template-aware**: each candidate then carries `template_support: {status: supported\|risky\|unsupported, reasons[], required_layout}` grounded in the template's canonical layouts, derivable layouts, font-aware placeholder capacities, and palette — and candidates needing absent layouts (or violating capacity) are demoted so they no longer rank first. With `template`, each candidate's `example` also carries `layout_id` (the layout it renders on — the matching layout for placeholder candidates, else One Content) and `layout_preview_png_path` (the shipped 320px thumbnail of that layout); placeholder candidates use it as `preview_png_path` (`renderer: "template-preview"`, `metadata_only: false`). Open the thumbnail to see the template before committing to a layout. |
...
**Stop when the quality gate passes.** Every `score_deck` response carries a `quality_gate` block — the **machine-readable definition of done**. `auto_repair` and `make_deck` apply the SAME criteria by default, so their `gate_passed` means what `quality_gate.passed` means (go-slide-creator-ie9v): a deck cannot converge in the repair loop and then be refused by the ship check. Overlong body text, nearly empty content, authored low-contrast highlights, and confirmed unreadably small text block both gates even when the numeric score passes. Accent overload, composition below 65, and problem-slide share above 40% are also fixed blockers in both tools. Predicted autofit shrinkage lowers the score but does not block by itself. Passing your own `gate` relaxes the configurable loop thresholds — and then `gate_passed` is a stop signal, not a shipping verdict; re-check with `score_deck` before you call it done.
...
**Native framework cards hug their content.** SWOT, PESTEL, KPI-dashboard and house diagrams measure their own text, shrink oversized sparse cards, and centre the result in the template's diagram region. A `sparse_layout` finding on one of these diagrams still means the text ink uses under 40% of that region; add meaningful supporting detail or use a shorter region. Its `add_detail_or_resize` advice is diagram-specific, not a shape-grid fix.

KPI dashboard trend arrows keep their theme accent, while their 10pt delta text is shaded to clear normal-text contrast against the card fill when that accent is too pale. The trend direction remains encoded by the arrow, not color alone.
...
**Response size.** Tool results carry the full payload as `structuredContent` and always include `content[0].text`. On protocol **2025-06-18 or later**, the text is a bounded (≤1 KB) JSON synopsis with status, paths, the first findings and next tool when available, so content-only hosts see an actionable result without duplicating large catalogs. Older clients still receive complete compact JSON. `--text-fallback=always` (or `JSON2PPTX_MCP_TEXT_FALLBACK=always`) forces the complete text copy; `never` forces the bounded synopsis. Image results keep each image in one native image block, not in the text synopsis.
...
| `blank-canvas` | none | Fully custom canvas with **no** title — all content is a `shape_grid`/`pattern`; no title band is reserved. The default content zone starts at the slide's 5% top margin, unless `headline` reserves a headline band. |
...
4. **REPAIR** — `validate_input` → `generate_presentation` → `render_deck_thumbnails` (all slides) → inspect every image (`inspect_slide_images` or your own review) → `repair_slide` → re-render. A raw `generate_presentation` response supplies a one-hour `deck_id`: pass it instead of the full presentation to `repair_slide`, `preview_presentation_plan`, `analyze_deck_rhythm`, `score_deck`, or `generate_presentation`. A handle-based repair updates the stored raw deck and returns `changed_slides` without `patched_deck` unless `return_deck:true`; stateless repair still returns the full deck. Images are truth; done means every slide of the current revision was rendered and inspected.
...
Pass `render_deck_spec` / `validate_deck_spec` / raw `repair_slide`'s `changed_slides` verbatim. The response carries
...
Exception for paired body columns: when both columns have content, native tables align to the top of their content zones so their first visible rows share a baseline. Charts keep their aspect ratio and horizontal centring but start at the content-zone top in any layout. A lone short table remains centred as described above.

**KPI `values` is always a JSON array of cells**, one per metric (`kpi-2up`…`kpi-6up`, `kpi-inline`). Each cell is either:

- an object `{"big": "$127M", "small": "Revenue"}` — `big` is the headline number (≤ 12 chars for full-size `kpi-Nup`, ≤ 8 for compact `kpi-inline`), `small` is the caption. The intuitive aliases `value`/`number` (→ `big`) and `label`/`caption` (→ `small`) are also accepted. An optional `sub` (≤ 12 chars) renders a small delta/trend annotation between the number and the caption (e.g. `{"big": "$50M", "small": "Revenue", "sub": "+5%"}`); the aliases `delta`/`trend`/`change` map to `sub`. Full-size cards emit `BODY_TOO_LONG` when a legal metric cannot fit on one line in its actual card. Put the sign in the value itself (`"+5%"` / `"-0.4%"`) — the annotation stays in the card's light text color rather than green/red so it remains legible on the accent fill.
...
Pie/donut format exception: slice labels default to calculated share percentages. Explicit `value_format.style: percent` formats those shares, so raw 40/60 values become 40%/60% without a scale warning. Explicit plain/compact/currency formats the raw slice values instead; prefix, suffix, and decimals apply to either mode. Pie and donut use the same contract.
...
**Semantic palette metadata.** Every shipped template exposes authored palette intent in `list_templates` (MCP) / `json2pptx skill-info` output. Compact discovery carries `semantic_accents`; use `fields=full` for the larger `surface_tints` and `data_palette` fields. A local custom template may omit this metadata:
...
**Canonical layout taxonomy.** The default compact discovery exposes concrete `canonical_layout_ids`; legacy `mode="compact"` and `fields="full"` add per-layout taxonomy matching `examine_template`, so you can plan against stable roles instead of raw layout names:
...
- With legacy `mode="compact"` or `fields="full"`, each `layout_summaries[]` entry carries `canonical_type` (e.g. `"Title Slide"`, `"One Content"`, `"Section Divider"`) and per-placeholder `role` (`title`, `eyebrow`, `section_number`, `body`, `image`, `chart`, `footer`, `date`, `page_number`, …) alongside `max_chars`. The default `fields="compact"` omits summaries. For a **title** placeholder `max_chars` is the measured capacity at the comfort size for the resolved font — the same threshold the measured title check flags against — not an area estimate.
- In legacy `mode="compact"` or `fields="full"`, `canonical_coverage` reports each content-bearing family (`title-slide`, `section-divider`, `one-content`, `qa-closing`) as `{present, layouts[]}`. A family with `present: false` means decks needing that slide kind cannot resolve a native layout.
- In those detailed projections, `derivable_layouts` lists `[{name, ready, missing[]}]` for higher-level layouts the engine can synthesise (e.g. `two-content`, `full-image`, grid patterns). When `ready: false`, `missing` names the absent prerequisite.
...
- **Portable body typography.** Regular body placeholders use a density-based size range, preserving in-range template sizes and normalising outliers before autofit. `TEXT_SIZE_OFF_TARGET` at `info` records an automatic correction; at `review` it means the fitted text still falls outside the density target and should be shortened or split. `list_templates` reports the canonical content layout's original and nominal five-paragraph body sizes.
