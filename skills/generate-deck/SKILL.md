...
| Detect svggen-mcp contract drift | `get_capabilities` | Returns `{schema_version, tool_list:[{name, description}], chart_types:[], diagram_types:[], chart_capabilities:[{type, max_series, max_points, max_categories, label_strategy, density_behavior, status, ...}], diagram_capabilities:[{type, max_nodes, max_depth, overflow_behavior, required_fields, optional_fields, status, ...}], deprecations:[], features:{dry_render, structured_errors}}`. `chart_capabilities` / `diagram_capabilities` carry per-type renderer limits and behavior sourced from the same svggen package that backs `json2pptx-mcp.get_chart_capabilities` / `get_diagram_capabilities`, so you can fetch capability metadata directly from the renderer without a json2pptx round-trip. `schema_version` is sourced from the svggen library version (single source). Call once per session and compare `schema_version` to the value you cached; a change means the rendering or validation contract may have shifted. Distinct from `json2pptx-mcp.get_capabilities` — this one is scoped to the svggen registry. |
...
| **Detect API drift** — fetch `schema_version`, live tool list, deprecations, feature flags. Both `json2pptx-mcp` and `svggen-mcp` expose the same standardized fields (`tool_list`, `registry: {charts, diagrams, patterns}`, `vocabularies: {fix_kinds, finding_codes, ...}`, `deprecations`, `features`) so one parse path detects drift on both servers. | `get_capabilities` | (CLI inlines) |
...
| **Render one slide's resolved plan as an annotated wireframe (SVG + base64 PNG) without LibreOffice or ImageMagick** — paints the slide frame, layout placeholders (dashed blue), `shape_grid` cells (labelled `r,c`, kind, dimensions), occupancy %, and per-cell fit-finding badges (severity-coded: `REF`/`SHR`/`REV`/`INF`). Off-cell findings appear in a footer strip. Reuses the same plan-resolution path as `preview_presentation_plan` (so geometry matches what `generate_presentation` would produce) and renders via in-process `svggen` — no shell-outs. Inputs mirror `preview_presentation_plan` (`presentation` JSON) plus required `slide_index` (0-based) and optional `format` ∈ {`svg`, `png`, `both`} (default `both`) and `width_px` (default 960, clamped 320..2400). Returns `{index, svg, png_base64, width, height, cell_count, placeholder_count, finding_count, layout_id, layout_name, slide_type, warnings, errors}`. PNG is heavier than SVG; pass `format="svg"` when an SVG viewer is available. | `preview_slide_wireframe` | (MCP-only) |
...
| Score a generated deck (0-100 with structured findings) — accepts optional `slide_indices: [int]` to render + score only the listed slides for faster iteration on a single slide; `per_slide` is filtered to those indices, `summary.slide_count` still reflects the full deck, and `composition` is omitted in subset mode | `score_deck` | (CLI inlines) |
...
| **Inspect rendered slide images with Claude vision** — returns structured findings `{severity, category, description, location, source, suggested_fixes}` whose `suggested_fixes[]` are pre-mapped to `repair_slide` fix kinds. When `ANTHROPIC_API_KEY` is set the report uses vision (`mode:"vision"`, `source:"vision"`); when unset the tool degrades to a pure-Go heuristic pass (`mode:"heuristic"`, `source:"heuristic"`, all findings P3) that detects blank slides, edge-band overflow, and unexpected aspect ratios. Heuristic findings are advisory only. | `inspect_slide_images` | `testrand qa` |
| Render one slide to a PNG image (preferred over `pptx2jpg` shell-out) | `render_slide_image` | `pptx2jpg` |
| **Render one slide directly from its JSON** — skips full-deck generation by wrapping the slide in a synthetic single-slide deck, generating to a temp PPTX, and returning the PNG. Returns the same image envelope as `render_slide_image`. Cached by `(slide_json + template_content + density)` so repeat calls with unchanged inputs return instantly. Pass `overlay=true` (`--overlay` on the CLI) to composite a diagnostic overlay on top of the rendered PNG — shape_grid cell rectangles, density-band tints (info=blue, review=amber, shrink_or_split=orange, refuse=red), and fit-finding badges — so you can *see* the diagnostic without cross-referencing finding coordinates against the image manually. Use this when iterating on a single slide's design — `O(1)` cost regardless of deck size. | `render_slide_image_from_json` | `json2pptx render-slide-from-json` |
...
**Pagination (discovery tools).** `list_templates`, `list_patterns`, and `list_icons` accept optional `cursor` and `page_size` arguments (default page size 50, clamped to [1, 200]). The response envelope echoes `total_count` and `page_size` on every call, plus `next_cursor` when more entries remain (absent on the last page). Pass `next_cursor` back as the next call's `cursor` argument — treat it as an opaque token. `list_patterns` returns `{groups, total_count, page_size, next_cursor?}` (groups are rebuilt per page in the canonical category order). `list_icons` returns `{sets, total_count, page_size, next_cursor?}` (each per-set `count` reflects names on the current page; use `total_count` for the corpus total after the optional `search` filter). `list_templates` adds the pagination fields alongside its existing top-level wrapper. Invalid cursors / page_size values surface as structured `INVALID_PARAMETER` errors.
...
**Isolated diagram validation.** The separate `svggen-mcp` server exposes `validate_diagram` for checking a diagram payload in isolation. It returns `{valid: bool, errors?: [{pattern, path, code, message, fix, next_tool_call?}]}` — note the wrapping `valid`/`errors` envelope (distinct from `expand_pattern`, which returns `{pattern, version, bounds_source, shape_grid, occupancy, density_warnings}`). Per-error `fix.kind` values come from the chart enum: `align_series`, `truncate_or_split`, `replace_value`, `explicit_scale`, `reduce_items`. Per-error `next_tool_call` routes shape errors (`required`, `invalid_type`, `invalid_format`) to `get_diagram_schema` for schema discovery, and constraint errors (e.g. capacity, log-scale, series alignment) to `validate_diagram` so the agent can re-check after applying the fix. Invalid style payloads return a structured rejection instead of being silently ignored. Use when validating a chart/diagram before embedding it into a slide.
