...
| `expand_pattern` | PLAN | Per pattern slide — preview the resolved `shape_grid` plus `density_warnings`, `cell_budgets`, and `bounds_source`. Density is a **height** ratio (wrapped block height / available height, each paragraph measured at its own size), so >110% means the renderer will autofit-shrink the cell, not that text is clipped; aim for the 60–110% band, and treat `fit_overflow` (clipped even at the smallest shrink) and `TEXT_BELOW_READABLE_MIN` (shrunk below the readable floor) as the actual defects. |
...
3. **Density pre-flight.** For each pattern slide, call `expand_pattern` with your populated values to confirm density is in the 60–110% optimal band (a height ratio: >110% means the cell will be autofit-shrunk, <60% that it is mostly empty).
