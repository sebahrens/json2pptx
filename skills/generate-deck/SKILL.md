...
`render_deck_spec` parse failures (including an unknown slide kind) and unknown named templates return the shared `findings[]` envelope with a code and a replayable discovery call (`list_slide_kinds` or `list_templates`); the findings include `available[]`, and a close template typo also has `did_you_mean`. A spec that cannot be parsed receives no `deck_id` from either render or validate — correct the source and send it again. Other render failures retain the compact `{success:false,error,diagnostics[]}` response.
