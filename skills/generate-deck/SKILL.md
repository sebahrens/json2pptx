...
| `team` | `members` (1–8 → `team-bios` cards; aliases `people`, `team`) | `title`, `takeaway` | each member: string (a bare name) or `{name, role, bio?, photo_label?}`. Every card needs a `role`; name ≤60 chars, role ≤80, bio ≤220 (~2 lines), `photo_label` is the initials badge (≤8 chars, dropped if longer). Past 8 people or any budget it degrades to a bullet list |
