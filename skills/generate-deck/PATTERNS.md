...
For BMC, KPI grids, 2x2 matrices, timelines, card grids, icon rows, two-column comparisons, accent-banded panels (`stylish-panels`), strategy-house frameworks (`strategy-house` — objective banner + 3-5 pillars + foundation, with optional roof badges), executive SCQA summaries (`scqa-summary` — 4-row Situation / Complication / Questions / Answer narrative arc), and Porter-style value chains (`value-chain` — 4-10 step columns with per-step description and optional highlight), use json2pptx's named patterns. Named patterns expand to validated `shape_grid` structures at generation time, replacing ~600 tokens of boilerplate with ~100 tokens.
