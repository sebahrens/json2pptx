...
**Single-track roadmaps.** `phase-roadmap` accepts `values.parallel_tracks`: up to four cross-cutting workstreams (governance, change management, upskilling) that run alongside every phase, each one line of at most 90 characters. They render below the phase descriptions as full-width tinted bars with `values.parallel_label` (default "In parallel", up to 24 characters) at the left spanning them; the phase-box row gives up the height they need, so adding tracks does not push the descriptions or a callout off the slide. Use `roadmap-phased` instead when the workstreams have different activities in each phase.

**Two-track process grids.** `process-grid-2row` accepts optional `values.column_headers` (up to 24 characters each) and `values.outcomes` (up to 32 characters each). Each list needs exactly one entry per phase column, so `row1_phases`, `row2_phases` and the list must all have the same length; a mismatch fails validation. Headers render as a bold header row with an accent underline above both tracks; outcomes render as tinted accent pills beneath them (e.g. "5–10% wallet ↑"). Both rows are content-sized and take their height from the tracks, so keep phase labels short when using both.
