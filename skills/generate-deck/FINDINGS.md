...
**Sort invariant.** Every `findings` / `fit_findings` array is returned in `(severity desc, slide_index asc, code asc)` order across every tool that emits findings (`validate_input`, `preview_presentation_plan`, `generate_presentation`, `repair_slide`, `score_deck`). `findings[0]` is always the most important fix to address first. Deck-level findings (path doesn't match `/slides/N/...`) sort before slide 0 at equal severity. Ordering is deterministic across runs — safe to walk top-down. Every entry carries an explicit `severity` derived from its `action` (`refuse` → `error`, `shrink_or_split` → `warning`, everything else → `info`), the same on every tool, so filtering by severity gives one answer about one finding.
...
| `title_wraps` | A normal 2-line title in a box tall enough for it is silent; 3+ lines that fit are informational, while wrapping inside a one-line box needs review. **Escalates to `shrink_or_split`** (`fix.kind: shorten_title`, `fix.params: {current_chars, max_chars, fit_scale_pct}`) when the measured title only fits its resolved title box below the comfort size (80% of the template title size, or 32pt for larger display titles) or only with reduced line spacing. Measured with the inherited title style (size, all-caps, line spacing); canonical `layout_id`s resolved. This is the **single** title-length code: `validate` emits the identical finding (same code, path and message, deduped in the envelope), the `quality` score reports the same sentence, and neither the old 60-character fallback nor `HEADLINE_TOO_LONG` fires alongside it | `info` / `review` / `shrink_or_split` | `reduce_text` / `shorten_title` |
...
| `TITLE_TRUNCATED` (preflight) | A section-divider title cannot fit its resolved box at the 28pt floor with normal line spacing. Its full text is preserved; `fix.params.max_chars` is the measured shortening target | `refuse` | `shorten_title` |
...
| `CHROME_COLLISION` | Populated body/media placeholder intersects opaque master/layout side artwork by at least 0.25in. Rendering narrows tall side-art content zones, but the template layout itself needs review | `review` | — |
...
| `MISSING_TITLE` | A slide expected to make a point carries no title placeholder text, pattern title value, or blank-canvas `headline`. A content-bearing `blank-canvas` is an argument slide; set its slide-level `headline`. Truly empty interstitial blank slides remain exempt | `review` | `provide_value` |
| `SECTION_NUMBER_SEQUENCE_MISMATCH` | A numeric label authored into a section-number alias or fallback body slot contradicts the automatic 1-based divider sequence. Leading zeroes are normalized; non-numeric custom labels remain valid | `refuse` | `provide_value` |
...
| `PATTERN_CONTENT_MISMATCH` | The pattern is not the shape of the content in it: a timeline whose stops are all measurements against non-period labels (axes inverted), a 2×2 whose four quadrants are all dates, or a flow / pyramid whose every item is a short label carrying a figure. Each rule needs EVERY item to match, so one date among three metrics does not trip it; a bare number (`2026`) is a period, not a measure. `fix.params: {from, to}` names the pattern whose shape the content already has | `review` | `swap_pattern` |
| `CHART_SERIES_LENGTH_MISMATCH` | A chart series carries a different number of values than the chart has categories. The renderer plots what it has and leaves the rest of the categories empty, so a truncated array shipped as a confidently wrong chart. Emitted as an **error** by `validate_deck_spec` at `slides[i].chart.data.series[j].values`, and as a `refuse` fit finding by the raw-deck path. Categorical charts only — `scatter_chart` / `bubble_chart` carry point objects, and a time series has no categories | `refuse` | — |
| `CHART_VALUE_NOT_NUMERIC` | A chart value is a quoted figure (`"10"`), a null, or an object. The chart renders as an empty plot with a format-error label. Same surfaces and severity as the length mismatch; the path names the offending element (`…values[k]`) | `refuse` | — |
...
| `TEXT_EXCEEDS_SHAPE` | A word in a shape_grid shape (raw or pattern-expanded) exceeds its geometry's usable text width. One finding per slide, ranked by overflow ratio. Predictions below 2× are advisory because modest estimates can render after shrinkage; predictions at least 2× and measured pattern failures are blocking warnings. If `available_pt < minimum_glyph_pt`, shortening cannot work: follow `patch_path` to lower a pointed pattern's `max_height_pct`, change pointed step types to `step`, or widen/change a raw shape. No `repair_slide` call is offered for a geometry patch. | `review` (<2× predicted) / `shrink_or_split` (≥2× predicted or measured) | `widen_shape_text_area` (predicted) |
...
| `SLIDE_UNDERUSED` | The ink bounding box of the slide's grid (filled shapes, text blocks, media) covers too little of the safe content area; skipped when non-title placeholders carry content (a near-empty placeholder slide is `SLIDE_NEARLY_EMPTY`). The threshold is **45%** for a restrictive authored `bounds` / `max_height_pct` cap and **22%** for content-sized patterns or uncapped grids. `fix.params: {content_area_pct, threshold_pct, band_capped_by: author\|pattern\|none, hint}`. Agent action: `author` → loosen the cap; `pattern` → add detail, compose a supporting zone, or choose a denser pattern; `none` → add grid content or combine zones. Full-area explicit bounds are not a cap. | `review` | `add_detail_or_resize` |
...
| `TEXT_SIZE_OFF_TARGET` | Regular placeholder body text uses a density-based target (16–20pt through six paragraphs, 14–18pt for seven to nine, 12–16pt for ten or more). `info` records a template size normalised before autofit; `review` means the fitted result is outside the target and suggests `reduce_text` / split. Explicit sizes and display placeholders above 40pt are exempt. |
...
| `TITLE_TRUNCATED` | Section-divider title cannot fit at the 28pt floor with normal line spacing. The full text is preserved rather than ellipsized; `action: refuse` blocks strict generation. `fix.kind: shorten_title`, `fix.params: {current_chars, max_chars, min_font_pt}`. Shorten the heading to about `max_chars` or use a wider divider |
...
| `contrast_unresolved` | Placeholder text cannot be certified readable on its own fill: a gradient color fails WCAG AA, or a solid/gradient fill cannot be resolved against the visible canvas. `action: refuse`; no executable text-color fix is offered. Add a local backdrop or revise the template fill and inspect the render. Reported by validation and generation even without the full fit report. |
| `pagination_default_threshold` | Pagination used default threshold (no template capacity available) |
| `contrast_autofixed` | Render-time text color auto-replaced to reach WCAG AA for that text's size (4.5:1 normal; 3:1 at ≥18pt or ≥14pt bold). `action: info`. A `replace_color` fix with `{from,to,target:"text",path}` appears only when a simple raw shape-grid cell maps unambiguously to authored text; inherited, grouped, pattern, and ambiguous rendered text have no executable fix. The finding path still identifies the rendered surface. |
| `contrast_predicted` | Preflight prediction that text will be auto-replaced for WCAG AA. For authored raw shape-grid text, `fix.kind: replace_color` carries `{from,to,target:"text",path,predicted_replacement,replacement_mode,source}` and can be sent to `repair_slide`; for template-inherited placeholder and derived pattern/compose text, no source-level fix is offered. Transparent cells use the effective background; photo backgrounds without an opaque scrim are not guessed. `contrast_check: false` suppresses predictions and swaps. `action: info`. |
| `placeholder_remapped` | A content `placeholder_id` was resolved to a different usable placeholder on the layout. Emitted both pre-flight (`preview_presentation_plan` and `generate_presentation` with `fit_report:true` — one finding per `resolved_slides[].placeholders[]` with `remapped:true`) and at render time. `action: info`, `fix.kind: remap_placeholder`, `fix.params: {from, to}`. Path targets `/slides/{i}/content/{j}/placeholder_id`. Agent action: author the resolved id directly to avoid the implicit rewrite. Section `subtitle` is not remapped: its body slots may hold decorative numbers, so use a `shape_grid` text box or another layout. |
...
| `split_pattern` | Split a pattern-driven shape_grid into two slides at a computed row boundary. Useful for overflowing grids without changing the pattern. With `path` on a pattern slide, splits the `pattern.values[path]` array instead (slide 1 keeps the first `first` items, slide 2 the rest) — the fact-preserving alternative to `reduce_items`/`resize_list`. **Both halves must satisfy the pattern's own contract**: a split that would leave a half the pattern rejects (exec-summary needs 3 points, so a 4-point slide has no legal split) is refused with `code: "semantic_review_required"` naming the point it tried, rather than handing back a deck that fails to generate. | — | `first: int` (cells/items on slide 1; default = half), `title_part_2: string` (suffix; default `"(continued)"`), `path: string` (pattern.values array key) |
...
| `reduce_items` | Truncate a pattern-values array field to `max_items`. **Refused** (`code: "semantic_review_required"`) when a dropped item contains a number, unit, negation, or qualifier. The refusal proposes `split_pattern{path, first}` via `next_tool_call` **only when that split is legal for the pattern** — it prefers the point you asked for, falls back to the most balanced legal one, and when the pattern has no legal split at all (its own minimum item count) it says so and proposes nothing, because the old unconditional proposal led straight into a hard validation error. | `path: string`, `max_items: int` (> 0) | `confirm_semantic_change: bool` (bypass the guard) |
