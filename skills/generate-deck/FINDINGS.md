...
| `contrast_unresolved` | Placeholder text cannot be certified readable across its gradient: a stop or interpolated RGB color fails WCAG AA, or a stop/alpha cannot be resolved safely. `action: refuse`; no executable text-color fix is offered. Add a local backdrop or revise the whole gradient and inspect the render. Reported by validation and generation even without the full fit report. |
