...
| `contrast_predicted` | Preflight prediction (validate / preview) that text will be auto-replaced for WCAG AA: 4.5:1 for normal text, 3:1 for ≥18pt or ≥14pt bold. Uses the **same replacement algorithm** as the render-time pass, so `predicted_replacement` equals the `contrast_autofixed` color for the same resolved fg/bg/template. Includes transparent shape-grid cells against the effective slide/layout/master background; photo backgrounds without an opaque-enough scrim are not guessed. Same `fix.kind` (`replace_color`) as `contrast_autofixed`, with `fix.params.predicted_replacement`, `replacement_mode` (`flip` = neutral text or author-set placeholder background snapped to a readable theme color; `lerp` = other text darkened/lightened via EnsureContrast), and `source` (`shape_grid`, or `slide_background` for placeholder text on a background the slide sets itself). `contrast_check: false` suppresses predictions and swaps. `action: info` |
