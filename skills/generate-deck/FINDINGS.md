...
| `TEXT_EXCEEDS_SHAPE` | A word in a shape_grid shape (raw grid or pattern-expanded) is wider than the text area left by the shape's preset geometry minus insets (a `chevron` keeps `w − 2·notch`, diamonds/triangles `w/2`, ellipses `0.7·w`), so it breaks mid-word or is clipped — e.g. `numbered-step-strip` chevron labels. One finding per slide. `fix.params: {cells, word, required_pt, available_pt, geometry, hint}`. Agent action: shorten the label, lower the size, or use `rect` / `homePlate` / the `stacked-box` style **Two confidences:** the deterministic detector ESTIMATES the box and stays advisory (a 15%-over estimate can still render on one line); a pattern that has MEASURED the failure at its own readable floor — `numbered-step-strip` chevron labels after it has surrendered notch depth and type size — raises it as `shrink_or_split`, which FAILS the default quality gate. | `review` (predicted) / `shrink_or_split` (measured) | `reduce_text` |
