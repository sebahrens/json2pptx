...
Every projection includes canonical_layout_availability: {available:[canonical names], unavailable:[canonical names]} for the selected template. A valid canonical name is not necessarily available on every template. The detailed compact/full modes additionally include canonical_layout_ids (only available names → concrete layout ID), plus name, aspect_ratio, layout_count, sha256 (stable content hash), metadata_version, theme_colors (scheme→hex map), color_roles (primary_fill, secondary_fill, body_fill, body_text, white_text_safe_body for 4.5:1, white_text_safe_large for 3:1, white_text_safe as body-safe alias), title_font, body_font, semantic_accents (positive/negative/neutral→accent), surface_tints, data_palette, accent_usage_guide (when authored), canonical_coverage (per content-bearing family: present + covering layouts), derivable_layouts ([{name, ready, missing}]), layout_names, layout_summaries ([{id, name, canonical_type, placeholders[{id, type, role, max_chars}]}]), table_styles [{id,name}]. Full mode adds layouts with per-layout canonical_type/canonical_family/canonical_confidence, placeholder roles, font size and exact bounds.
