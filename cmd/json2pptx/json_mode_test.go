...
func TestConvertPresentationSlides_AutoLayout(t *testing.T) {
	...
	t.Run("auto-selects layout when layout_id is empty", func(t *testing.T) {
		...
		specs, err := convertPresentationSlides(slides, layouts, 0, 0, nil, nil)
		if err != nil {
	...
	t.Run("error when no layouts and no layout_id", func(t *testing.T) {
		...
		_, err := convertPresentationSlides(slides, nil, 0, 0, nil, nil)
		if err == nil {
...
func TestSectionSlideContentType(t *testing.T) {
	...
	specs, err := convertPresentationSlides(slides, []types.LayoutMetadata{sectionLayout}, 0, 0, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(specs) != 1 {
