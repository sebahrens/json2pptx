...
var outputSchemaListTemplates = json.RawMessage(`{
  ...
  "properties": {
    ...
    "templates":       {"type": "array", "items": {"type": "object", "properties": {
      "canonical_layout_availability": {"type": "object", "description": "Template-specific canonical aliases in every projection; unavailable names must not be pinned as layout_id.", "properties": {
...
var outputSchemaAutoRepair = json.RawMessage(`{
  ...
  "properties": {
    ...
    "deterministic_ready": {"type": "boolean", "description": "Everything the engine can decide on its own: the deterministic gate and structural checks passed on complete evidence and the content is author-supplied. This is the flag the DEFAULT path can reach — publishable additionally needs a visual verdict the default path never produces. Equivalent to deterministic_blocking_reasons being empty."},
    "publishable": {"type": "boolean", "description": "Full ship-as-is flag: deterministic_ready AND a current all-slide vision inspection with an explicit approved verdict. UNREACHABLE on the default path, which renders no pixels: call render_deck_thumbnails, look at every slide, then submit_visual_review. Equivalent to blocking_reasons being empty. Gate on deterministic_ready when deciding whether the deck still needs editing."},
    ...
    "blocking_reasons": {"type": "array", "items": {"type": "string"}, "description": "Every reason the deck is not publishable — unmet gate criteria (incl. final output validation), incomplete evidence, exemplar provenance, and the missing visual verdict. Present only when publishable=false. Superset of gate_reasons and of deterministic_blocking_reasons."},
    "deterministic_blocking_reasons": {"type": "array", "items": {"type": "string"}, "description": "blocking_reasons minus the visual-review entry: everything an agent can still fix by editing the deck. Present only when deterministic_ready=false."},
    "idempotent_replay": {"type": "boolean", "description": "True when this response was served from the idempotency cache (the caller passed an idempotency_key that matched a prior successful call)."}
  },
...
var outputSchemaRenderDeckSpec = json.RawMessage(`{
  ...
  "properties": {
    ...
    "deterministic_ready": {"type": "boolean", "description": "Author-supplied content passed the quality gate and structural validation on complete evidence. This is the flag a fresh render can reach."},
    "publishable":     {"type": "boolean", "description": "Full ship-as-is verdict: deterministic_ready AND an approved all-slide visual review of the current artifact. False on a fresh render."},
    "manual_review_required": {"type": "boolean", "description": "True until the current artifact has a complete approved visual verdict; inverse of publishable."},
    "blocking_reasons": {"type": "array", "items": {"type": "string"}, "description": "All reasons publishable is false, including the missing or stale visual verdict."},
