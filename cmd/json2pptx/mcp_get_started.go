...
func buildGetStartedResponse(task string) getStartedResponse {
	...
	case "brief":
		seq = []getStartedStep{
			...
			{Tool: "preview_presentation_plan", WhenToCall: "Dry-run the assembled deck JSON to verify layout selection, placeholder mapping, and fit findings without rendering."},
			{Tool: "generate_presentation", WhenToCall: "Produce the PPTX once preview is clean. Pass strict_fit: \"warn\" (default) or \"strict\" for refuse-on-overflow."},
	...
	case "revise":
		seq = []getStartedStep{
			...
			{Tool: "read_presentation", WhenToCall: "Read the existing PPTX back as structured JSON to see what is actually on each slide."},
			{Tool: "preview_presentation_plan", WhenToCall: "Re-validate the deck JSON against the current engine; surfaces fit findings and warnings."},
			{Tool: "repair_slide", WhenToCall: "Apply targeted fixes (the Fix.Kind vocabulary fit-report emits) per slide that has findings."},
			{Tool: "generate_presentation", WhenToCall: "Regenerate the PPTX after repairs."},
		...
		notes = []string{
			...
			"If you do not have the deck JSON, read_presentation gives a best-effort reconstruction.",
		}
	case "validate-only":
		seq = []getStartedStep{
...
- "brief" (default): authoring a new deck — get_capabilities → list_templates → plan_deck → recommend_visual → preview_presentation_plan → generate_presentation → score_deck.
- "revise": modifying an existing PPTX — get_capabilities → read_presentation → preview_presentation_plan → repair_slide → generate_presentation → score_deck.
