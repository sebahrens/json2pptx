	...
	case "brief":
		return &getStartedFastPath{
			...
			Steps: []getStartedStep{
				{Tool: "list_slide_kinds", WhenToCall: "Pick a kind per slide; copy its example and match its item_schema exactly (unknown fields are reported as SEMANTIC_UNKNOWN_FIELD). Its `compositions` list is what a slide's optional pattern/layout override will honour — anything else is ignored and reported as SEMANTIC_PATTERN_NOT_AVAILABLE. A kind reaches 23 of the 43 patterns; for one of the other 20 (pull-quote, swimlane, pyramid, value-chain, waterfall-bridge, strategy-house, scqa-summary and the rest — SKILL.md lists them all) use kind raw_json2pptx and carry the pattern block verbatim, as its example shows."},
