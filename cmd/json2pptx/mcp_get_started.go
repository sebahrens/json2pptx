...
func fastPathFor(task string, seq []getStartedStep) *getStartedFastPath {
	...
	case "revise":
		...
		return &getStartedFastPath{
			Tool:       "render_deck_spec",
			WhenToCall: "RECOMMENDED PATH when the deck was authored as a DeckSpec (task=brief) — you do not resend it. Every validate_deck_spec / render_deck_spec response carries a deck_id: the spec this server is holding. Send deck_id INSTEAD of spec, with patch:[{op:\"replace\", path:\"/slides/3/title\", value:\"…\"}] — op is replace | add | remove, path is a JSON Pointer into the spec (/meta/template to restyle the deck, /slides/6 with add to insert a slide, /slides/2 with remove to drop one). Findings' next_tool_call offers a semantic patch when a single text field can be identified. A four-edit revision is one call of a few hundred bytes instead of a full spec re-upload. The response's changed_slides names the 0-based slides that differ, so render_deck_thumbnails them as slide_indices instead of pulling the whole deck again. score_deck and analyze_deck_rhythm also accept deck_id. repair_slide accepts deck_id only as a raw escape hatch: it returns patched raw JSON and does NOT update the stored semantic spec; use deck_id + patch for durable DeckSpec repairs. Handles live 1 hour per server process; if one expires, send the spec again. Still holding the spec and no handle? Edit it and call render_deck_spec — findings come back at semantic_path, so you fix the field the finding names. The raw chain in `sequence` is for a deck authored as raw json2pptx JSON, not as a DeckSpec.",
			Steps: []getStartedStep{
...
func buildGetStartedResponseOpts(task string, rt getStartedRuntime, verbose bool) getStartedResponse {
	...
	case "revise":
		...
		seq = append(seq, []getStartedStep{
			...
			{Tool: "preview_presentation_plan", WhenToCall: "Dry-run the deck JSON, or use the raw deck_id from generate_presentation, to surface per-slide fit findings whose Fix.Kind directives feed repair_slide."},
			{Tool: "repair_slide", WhenToCall: "Apply targeted fixes using the raw deck_id and Fix.Kind directives. The stored deck updates and changed_slides names what to re-render; return_deck:true is optional. A stateless call may instead pass the full deck JSON."},
			{Tool: "generate_presentation", WhenToCall: "Regenerate with the raw deck_id if one exists; after a stateless repair, pass its patched_deck. This call returns a raw deck_id for later revisions."},
		...
		notes = []string{
			"TWO REVISE PATHS, and the one you want depends on how the deck was authored. (1) DeckSpec deck — fast_path: deck_id + patch on validate_deck_spec / render_deck_spec. The server holds the spec, you send the edit, changed_slides tells you which thumbnails to re-pull. (2) Raw json2pptx deck — validate_input on the authored JSON. If a prior generate_presentation returned a live raw deck_id, pass it to preview_presentation_plan, repair_slide, score_deck, analyze_deck_rhythm, and generate_presentation; repair_slide updates it and returns changed_slides without echoing the full deck unless return_deck:true. Without a live handle, pass JSON to stateless repair and generate once to obtain one. Do not use path 2 on a DeckSpec deck: repair_slide edits compiled slide JSON, so its fixes do not travel back into the spec and are lost on the next render_deck_spec.",
			"A deck_id is per server process and expires one hour after its last stored revision; reading it does not refresh the TTL. It is not storage: if the server restarts or the handle expires, send the authoritative JSON or spec again for a new one. Keep your own copy — the handle saves bytes, it is not the deck's home.",
			...
			"Supply the authoritative raw deck JSON to validate_input. After generation, a live raw deck_id can stand in for that JSON on preview_presentation_plan, repair_slide, score_deck, analyze_deck_rhythm, and generate_presentation. read_presentation is a verification aid only — it does not reconstruct a PresentationInput.",
