...
`generate_presentation` (MCP) and `json2pptx generate` (CLI) default to **strict** output validation: every successful response implies the generated `.pptx` passed the full OPC + OOXML validator (`internal/pptx.ValidateOutputFile`) with zero blocking findings. This is the "zero needs repair" contract — PowerPoint and Keynote will not show the *"we found a problem with some content"* prompt on a strict-mode success. When validation fails, the response is an error envelope `{summary, findings[], next_tool_call: {tool: "repair_slide", args_template}}` carrying `OPC_*` / `OOXML_*` findings with per-finding `scope` (`source` / `template` / `generator`). See [the raw authoring path](skills/generate-deck/RAW_PATH.md#diagnose-and-repair) for the response protocol; use `describe_finding` for the current code catalog.
...
If `templates/p-style.pptx` exists locally, the test suite includes it in the template corpus without tracking or embedding it.
