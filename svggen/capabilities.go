...
func ChartCapabilities() []ChartCapability {
	caps := []ChartCapability{
		...
		},
		{
			...
			LabelStrategy:     strPtr("adapt: shrink→two-line wrap→rotate up to 90°→bounded ellipsis; never thin nominal categories"),
			DensityBehavior:   strPtr("crowded categories recommend horizontal-bar-with-callouts or split; log-scale disabled for stacked"),
			Status:            "ready",
		},
		{
			Type:              "line",
			...
			SupportsLogScale:  boolPtr(false),
			LabelStrategy:     strPtr("adapt: shrink→two-line wrap→rotate up to 90°→bounded ellipsis; never thin nominal categories"),
			DensityBehavior:   strPtr("crowded categories recommend horizontal-bar-with-callouts or split"),
			Status:            "ready",
		},
		{
			Type:              "area",
