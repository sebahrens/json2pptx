...
func validatePillars(path string, slide SlideSpec, s *semDiags) {
	field, problem, hard := slides.PillarsIssue(slide.Body)
	if problem == "" || (field == "pillars" && !hasNonEmpty(slide.Body, "pillars")) {
		return
	}
	if hard {
