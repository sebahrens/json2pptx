...
var codeMetaRegistry = map[string]patterns.FindingMeta{
	"navigation_requires_authoring":   briefMeta("navigation_requires_authoring", "Bullet continuation cannot safely shift numeric slide destinations.", "split_bullets would expand a deck containing internal numeric slide links, or navigation cannot be inspected safely.", "Author sibling slides explicitly and update numeric destinations for every internal slide link; preserve all source text and render every resulting page. Do not bypass the navigation refusal.", describeSeverityRefuse),
	...
	"invalid_color":                   briefMeta("invalid_color", "A pattern override or theme_override has an invalid color.", "Validation rejects a pattern fill/line color, or a theme_override.colors value that is not six hex digits.", "Use a six-digit hex color (#RRGGBB); pattern overrides also accept a supported theme scheme color name.", describeSeverityRefuse),
	...
	CodeSemanticUnknownField: {
		...
		Severity:    describeSeverityRefuse,
		WhenEmitted: "semantic parsing finds a top-level key other than meta or slides (most often a stale spec using deck instead of meta), or validation finds a slide payload key, list-entry key (e.g. slides[i].kpis[j].valeu), or chart key that the kind's compiler never reads. This is a blocking error at every strictness level because the content would otherwise be dropped silently. The path names the dropped key; fix.params.did_you_mean names the likely intended key.",
		RemediationSteps: []string{
