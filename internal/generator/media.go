...
// conflicts for natural-aspect diagram types no longer arise: an explicit
// output box now wins over a diagram's natural aspect (go-slide-creator-rkq0).
//
...
func (ctx *singlePassContext) resolveDiagramWithMetadata(slideNum int, item ContentItem, placeholderBounds types.BoundingBox) (*DiagramRenderResult, bool) {
	...
	rendered, err := renderDiagramSpecFull(diagramSpec, ctx.themeColors, maxPNGWidth, svgOnly, ctx.strictFit, ctx.viewingMode, nil)
	if err != nil {
