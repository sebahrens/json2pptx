		...
		t.Run(strings.TrimSuffix(filepath.Base(path), ".pptx"), func(t *testing.T) {
			...
			for _, existing := range []bool{false, true} {
				t.Run(fmt.Sprintf("existing=%v", existing), func(t *testing.T) {
					...
					req := GenerationRequest{TemplatePath: path, OutputPath: output, StrictFit: "strict", ExcludeTemplateSlides: true, Slides: []SlideSpec{{LayoutID: layoutID, Content: []ContentItem{{PlaceholderID: slot.ID, Type: ContentTable, Value: table}}}}}
