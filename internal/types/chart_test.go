...
func TestLayoutPresets(t *testing.T) {
	tests := []struct {
		...
		{PresetSlide16x9, 1920, 1080},
		{PresetContent16x9, 1600, 900},
		{PresetHalf16x9, 760, 720},
		{PresetThird16x9, 500, 720},
		{PresetSlide4x3, 1024, 768},
		{PresetHalf4x3, 420, 540},
		{PresetSquare, 600, 600},
		{PresetThumbnail, 400, 300},
	}

	for _, tt := range tests {
		t.Run(string(tt.preset), func(t *testing.T) {
			dims, ok := LayoutPresets[tt.preset]
			if !ok {
				t.Fatalf("LayoutPresets[%q] not found", tt.preset)
			}
			if dims.Width != tt.expectWidth {
				t.Errorf("Width = %d, want %d", dims.Width, tt.expectWidth)
			}
			if dims.Height != tt.expectHeight {
				t.Errorf("Height = %d, want %d", dims.Height, tt.expectHeight)
			}
		})
	}
}

func TestGetPresetDimensions(t *testing.T) {
	t.Run("valid presets", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.preset, func(t *testing.T) {
				width, height, ok := GetPresetDimensions(tt.preset)
				if !ok {
					t.Fatalf("GetPresetDimensions(%q) returned ok=false", tt.preset)
				}
				if width != tt.expectWidth {
					t.Errorf("width = %d, want %d", width, tt.expectWidth)
				}
				if height != tt.expectHeight {
					t.Errorf("height = %d, want %d", height, tt.expectHeight)
				}
			})
		}
	})
	...
	t.Run("invalid preset", func(t *testing.T) {
		width, height, ok := GetPresetDimensions("nonexistent_preset")
		if ok {
			t.Error("GetPresetDimensions() returned ok=true for invalid preset")
		}
		if width != 0 || height != 0 {
			t.Errorf("GetPresetDimensions() = (%d, %d), want (0, 0) for invalid preset", width, height)
		}
	})

	t.Run("empty preset", func(t *testing.T) {
		width, height, ok := GetPresetDimensions("")
		if ok {
			t.Error("GetPresetDimensions() returned ok=true for empty preset")
		}
		if width != 0 || height != 0 {
			t.Errorf("GetPresetDimensions() = (%d, %d), want (0, 0) for empty preset", width, height)
		}
	})
}

func TestPresetConstants(t *testing.T) {
	// Verify that all preset constants exist in the map
	...
	for _, p := range presets {
		if _, ok := LayoutPresets[p]; !ok {
			t.Errorf("LayoutPreset constant %q not found in LayoutPresets map", p)
		}
	}

	// Verify we have exactly the expected number of presets
	if len(LayoutPresets) != len(presets) {
		t.Errorf("LayoutPresets has %d entries, expected %d", len(LayoutPresets), len(presets))
	}
}

func TestDetectLayoutPreset(t *testing.T) {
	// Standard 16:9 slide width is 9144000 EMUs (10 inches)
	slideWidth16x9 := EMU(StandardSlideWidth16x9)

	// Helper function for percentage calculations
	pct := func(percent float64) EMU {
		return EMU(int64(float64(slideWidth16x9) * percent / 100))
	}

	tests := []struct {
		name             string
		placeholderWidth EMU
		slideWidth       EMU
		expected         LayoutPreset
	}{
		// Full width cases (>80% of slide width)
		{
			name:             "full width placeholder (100%)",
			placeholderWidth: slideWidth16x9,
			slideWidth:       slideWidth16x9,
			expected:         PresetContent16x9,
		},
		{
			name:             "full width placeholder (90%)",
			placeholderWidth: pct(90),
			slideWidth:       slideWidth16x9,
			expected:         PresetContent16x9,
		},
		{
			name:             "boundary full width (81%)",
			placeholderWidth: pct(81),
			slideWidth:       slideWidth16x9,
			expected:         PresetContent16x9,
		},

		// Half width cases (35%-80% of slide width)
		{
			name:             "half width placeholder (50%)",
			placeholderWidth: slideWidth16x9 / 2,
			slideWidth:       slideWidth16x9,
			expected:         PresetHalf16x9,
		},
		{
			name:             "boundary half width (79%)",
			placeholderWidth: pct(79),
			slideWidth:       slideWidth16x9,
			expected:         PresetHalf16x9,
		},
		{
			name:             "boundary half width (36%)",
			placeholderWidth: pct(36),
			slideWidth:       slideWidth16x9,
			expected:         PresetHalf16x9,
		},

		// Third width cases (<35% of slide width)
		{
			name:             "third width placeholder (33%)",
			placeholderWidth: slideWidth16x9 / 3,
			slideWidth:       slideWidth16x9,
			expected:         PresetThird16x9,
		},
		{
			name:             "third width placeholder (25%)",
			placeholderWidth: slideWidth16x9 / 4,
			slideWidth:       slideWidth16x9,
			expected:         PresetThird16x9,
		},
		{
			name:             "boundary third width (34%)",
			placeholderWidth: pct(34),
			slideWidth:       slideWidth16x9,
			expected:         PresetThird16x9,
		},
		...
		// Edge cases
		{
			name:             "zero slide width (fallback)",
			placeholderWidth: slideWidth16x9 / 2,
			slideWidth:       0,
			expected:         PresetContent16x9,
		},
		{
			name:             "negative slide width (fallback)",
			placeholderWidth: slideWidth16x9 / 2,
			slideWidth:       -1000,
			expected:         PresetContent16x9,
		},
		{
			name:             "tiny placeholder (10%)",
			placeholderWidth: slideWidth16x9 / 10,
			slideWidth:       slideWidth16x9,
			expected:         PresetThird16x9,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := DetectLayoutPreset(tt.placeholderWidth, tt.slideWidth)
			if result != tt.expected {
				ratio := float64(tt.placeholderWidth) / float64(tt.slideWidth)
				t.Errorf("DetectLayoutPreset(%d, %d) = %q, want %q (ratio=%.2f)",
					tt.placeholderWidth, tt.slideWidth, result, tt.expected, ratio)
			}
		})
	}
}

func TestDetectLayoutPresetWithDimensions(t *testing.T) {
	slideWidth := EMU(StandardSlideWidth16x9)
	slideHeight := EMU(5143500) // Standard 16:9 height (5.625 inches = 10" / 16 * 9)

	// Helper for aspect ratio calculations
	aspectDiv := func(width EMU, divisor float64) EMU {
		return EMU(int64(float64(width) / divisor))
	}

	tests := []struct {
		name              string
		placeholderWidth  EMU
		placeholderHeight EMU
		slideWidth        EMU
		slideHeight       EMU
		expected          LayoutPreset
	}{
		{
			name:              "square placeholder (pie/donut chart)",
			placeholderWidth:  2000000,
			placeholderHeight: 2000000,
			slideWidth:        slideWidth,
			slideHeight:       slideHeight,
			expected:          PresetSquare,
		},
		{
			name:              "near-square placeholder (aspect 0.95)",
			placeholderWidth:  2000000,
			placeholderHeight: aspectDiv(2000000, 0.95),
			slideWidth:        slideWidth,
			slideHeight:       slideHeight,
			expected:          PresetSquare,
		},
		{
			name:              "near-square placeholder (aspect 1.05)",
			placeholderWidth:  2000000,
			placeholderHeight: aspectDiv(2000000, 1.05),
			slideWidth:        slideWidth,
			slideHeight:       slideHeight,
			expected:          PresetSquare,
		},
		{
			name:              "wide placeholder falls through to width detection",
			placeholderWidth:  slideWidth / 2,
			placeholderHeight: slideWidth / 4, // 2:1 aspect ratio
			slideWidth:        slideWidth,
			slideHeight:       slideHeight,
			expected:          PresetHalf16x9,
		},
		{
			name:              "tall narrow placeholder falls through to width detection",
			placeholderWidth:  slideWidth / 4,
			placeholderHeight: slideWidth / 2, // 1:2 aspect ratio
			slideWidth:        slideWidth,
			slideHeight:       slideHeight,
			expected:          PresetThird16x9,
		},
		{
			name:              "zero height falls through to width detection",
			placeholderWidth:  slideWidth,
			placeholderHeight: 0,
			slideWidth:        slideWidth,
			slideHeight:       slideHeight,
			expected:          PresetContent16x9,
		...
		{
			name:              "zero width falls through to width detection",
			placeholderWidth:  0,
			placeholderHeight: 2000000,
			slideWidth:        slideWidth,
			slideHeight:       slideHeight,
			expected:          PresetThird16x9, // 0/slideWidth < 0.35
		},
		// 4:3 slide detection (standard 4:3: 9144000 x 6858000 EMU)
		{
			name:              "4:3 slide full width returns 4:3 preset",
			placeholderWidth:  9144000,
			placeholderHeight: 3000000,
			slideWidth:        9144000,
			slideHeight:       6858000,
			expected:          PresetSlide4x3,
		},
		{
			name:              "4:3 slide half width returns 4:3 half preset",
			placeholderWidth:  4572000, // 50% of 9144000
			placeholderHeight: 3000000,
			slideWidth:        9144000,
			slideHeight:       6858000,
			expected:          PresetHalf4x3,
		},
		{
			name:              "4:3 slide third width returns third 16:9 (no 4:3 third preset)",
			placeholderWidth:  2743200,                   // 30% of 9144000
			placeholderHeight: 5000000,                   // wide enough to avoid square detection
			slideWidth:        9144000,                   // 10"
			slideHeight:       6858000,                   // 7.5" (4:3)
			expected:          PresetThird16x9,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := DetectLayoutPresetWithDimensions(
				tt.placeholderWidth, tt.placeholderHeight,
				tt.slideWidth, tt.slideHeight,
			)
			if result != tt.expected {
				t.Errorf("DetectLayoutPresetWithDimensions() = %q, want %q", result, tt.expected)
			}
		})
	}
}

func TestIsNear4x3Ratio(t *testing.T) {
	tests := []struct {
		name     string
		width    EMU
		height   EMU
		...
		{"standard 4:3 (9144000 x 6858000)", 9144000, 6858000, true},
		{"standard 16:9 (12192000 x 6858000)", 12192000, 6858000, false},
		{"same-width 16:9 (9144000 x 5143500)", 9144000, 5143500, false},
		{"zero width", 0, 6858000, false},
		{"zero height", 9144000, 0, false},
		{"near 4:3 within tolerance", 9144000, 6900000, true}, // ratio ~1.325, close to 1.333
		{"far from 4:3", 9144000, 4000000, false},             // ratio ~2.286
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := isNear4x3Ratio(tt.width, tt.height)
			if result != tt.expected {
				ratio := float64(tt.width) / float64(tt.height)
				t.Errorf("isNear4x3Ratio(%d, %d) = %v, want %v (ratio=%.4f)",
					tt.width, tt.height, result, tt.expected, ratio)
			}
		})
	}
}

func TestStandardSlideWidthConstants(t *testing.T) {
	// Verify the standard slide width constants are correct
	// 10 inches = 10 * 914400 EMUs = 9144000 EMUs
	expectedWidth := EMU(10 * 914400)

	if StandardSlideWidth16x9 != expectedWidth {
		t.Errorf("StandardSlideWidth16x9 = %d, want %d", StandardSlideWidth16x9, expectedWidth)
	}
	if StandardSlideWidth4x3 != expectedWidth {
		t.Errorf("StandardSlideWidth4x3 = %d, want %d", StandardSlideWidth4x3, expectedWidth)
	}
}
...
func TestCalculateDynamicScale(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			placeholderWidthEMU: StandardSlideWidth16x9, // 10 inches
